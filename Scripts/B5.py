...
i = Image.open("./Images/hall-mod_0.bmp") #Image initiale
sortie = i.copy()
...
sortie.save("./Images/Imageout_steg_0.bmp")  #Image modifiée

#On créer une matrice selon la taille de l'image à cacher
i = Image.open("./Images/Imageout3.bmp") #Image à cacher
matrice = []
for x in range(i.size[0]): #Ligne
    matrice.append([])
    for y in range(i.size[1]): #Colonne
...
#On créer une matrice selon la taille de l'image à décoder
i = Image.open("./Images/Imageout_steg_0.bmp")
matrice = []
for x in range(i.size[0]): #Ligne
    matrice.append([])
...
#On obtient les valeurs de l'image cachée que l'on ajoute dans la matrice
for x in range(i.size[0]): #Ligne
    for y in range(i.size[1]): #Colonne
        c = i.getpixel((x,y))
        if c[0]%2 == 1:
